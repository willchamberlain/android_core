 ...
 *
 * fixed-size array(s) - one per data element : initially empty
 *
...
class DataTrack {
    ...
    public synchronized void add(DetectedFeature detectedFeature){  //  TODO: not efficient - look at circular lists / circular queues
        if(sizeNow < size) {
            ...
            for (int i_ = 1; i_ < sizeNow; i_++) {       //  e.g. if size = 10    1 ... 9
                newData[i_ - 1] = data[i_];              //  e.g. if size = 10    newData[0]=data[1] ... newData[8]=data[9]
                newData[sizeNow-1] = detectedFeature;    //  e.g. if size = 10    newData[9]=detectedFeature
                data = newData;
            }
        }
    }
