...
//    public void matlabPoseEstimation()
    public void estimatePose(List<Observation2> observation2List, SmartCameraExtrinsicsCalibrator calibrator) {
        ...
        connect_to_POSE_FROM_2D3D_SERVICE_ROS();  // todo - CAN FAIL
...
//        int u_=100, v_=200; //        double x_=11, y_=22, z_=33;//        createObs(request, u_, v_, x_, y_, z_);
        ...
        createObs(request,     182.8574242089062d , 108.62255574234761d, 3.92d ,  0.00d , 0.252d  );  // 2 - 357
        createObs(request,     182.41568510974201d , 76.8101381190807d,  3.92d ,  0.00d , 0.645d  );  // 2 - 257

        createObs(request,     285.879892804077d ,  108.32705433683117d, 3.92d , -1.28d , 0.252d  );  // 3 - 357
        createObs(request,     288.944522535763d ,   75.79036791913009d, 3.92d , -1.28d , 0.645d  );  // 3 - 257
...
//        createObs(request,     312.2390321889994d , 214.18113331292741d, 1.36d , -0.64d , 0.645d  );  // 5 - 357
        ...
        createObs(request,     246.04851963779973d ,214.0295061185083d,  1.36d , -0.32d , 0.252d  );  // 6 - 357
        createObs(request,     251.05223417229467d ,136.82524151860272d, 1.36d , -0.32d , 0.645d  );  // 6 - 257

        createObs(request,     180.1327822481631d , 213.4655434325827d,  1.36d ,  0.00d , 0.252d  );  // 7 - 357
        createObs(request,     179.24505599894138d ,136.39673251911438d, 1.36d ,  0.00d , 0.645d  );  // 7 - 257

        createObs(request,     50.91477856287188d , 212.58922727425147d, 1.36d ,  0.64d , 0.252d  );  // 8 - 357
        createObs(request,     39.748161293765655d ,136.19683774723288d, 1.36d ,  0.64d , 0.645d  );  // 8 - 257

        createObs(request,     37.093278687336245d ,138.83215031041536d, 2.64d ,  1.28d , 0.252d  );  // 9 - 357
        createObs(request,     29.93565957697492d ,  94.52635225774982d, 2.64d ,  1.28d , 0.645d  );  // 9 - 257

        createObs(request,     182.30592540117604d, 138.4954591343754d,  2.64d ,  0.00d , 0.252d  );  //10 - 357
        createObs(request,     181.84473615057965d , 93.39524995177273d, 2.64d ,  0.00d , 0.645d  );  //10 - 257
    ...
    private boolean connect_to_POSE_FROM_2D3D_SERVICE_ROS() {
        try {
            if(null !=poseFrom2D3D_serviceClient && poseFrom2D3D_serviceClient.isConnected()) {
                return true;
            } else if(null !=poseFrom2D3D_serviceClient && !poseFrom2D3D_serviceClient.isConnected()) {
                System.out.println("WARN: VosTaskAssignmentSubscriberNode: connect_to_POSE_FROM_2D3D_SERVICE_ROS: poseFrom2D3D_serviceClient is disconnected: reconnecting");
                make_connection_to_POSE_FROM_2D3D_SERVICE_ROS();
                return true;
            } else {
                System.out.println("INFO: VosTaskAssignmentSubscriberNode: connect_to_POSE_FROM_2D3D_SERVICE_ROS: poseFrom2D3D_serviceClient was null: connecting");
                make_connection_to_POSE_FROM_2D3D_SERVICE_ROS();
                return true;
            }
        } catch (ServiceNotFoundException e) {
            System.out.println("ERROR: VosTaskAssignmentSubscriberNode: onStart: poseFrom2D3D: ServiceNotFoundException for '"+POSE_FROM_2D3D_SERVICE_ROS_NAME +"': "+e.getMessage());
            e.printStackTrace();
//            throw new RosRuntimeException(e);
        } catch (Throwable e) {
            if (connectedNode != null) {
                System.out.println("ERROR: VosTaskAssignmentSubscriberNode: onStart: poseFrom2D3D: Exception for '" + POSE_FROM_2D3D_SERVICE_ROS_NAME + "': " + e.getMessage());
                e.printStackTrace();
//                throw new RosRuntimeException(e);
            } else {
                System.out.println("ERROR: VosTaskAssignmentSubscriberNode: onStart: poseFrom2D3D: connectedNode_ == null: " + e.getMessage());
                e.printStackTrace();
            }
        }
        return false;
    ...
    private void make_connection_to_POSE_FROM_2D3D_SERVICE_ROS() throws ServiceNotFoundException {
